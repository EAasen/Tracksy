...
describe('Auth Flow', function() {
  ...
  it('should login a user and rotate refresh token', async () => {
    await request(app).post('/api/v1/auth/signup').send({ username: 'bob', password: 'Password123!', email: 'bob@example.com' });
    const login = await request(app).post('/api/v1/auth/login').send({ username: 'bob', password: 'Password123!' }).expect(200);
    login.body.should.have.property('accessToken');
    login.body.should.have.property('refreshToken');
    login.body.should.have.property('user');
  });

  it('should refresh access token', async () => {
    const signup = await request(app).post('/api/v1/auth/signup').send({ username: 'carol', password: 'Password123!', email: 'carol@example.com' });
    const refresh = await request(app).post('/api/v1/auth/token/refresh').send({ refreshToken: signup.body.refreshToken }).expect(200);
    refresh.body.should.have.property('accessToken');
  });

  it('should request password recovery and reset password', async () => {
    const signup = await request(app).post('/api/v1/auth/signup').send({ username: 'dave', password: 'Password123!', email: 'dave@example.com' });
    await request(app).post('/api/v1/auth/password-recovery').send({ email: 'dave@example.com' }).expect(200);
    ...
    await request(app).post('/api/v1/auth/password-reset').send({ token: raw, password: 'NewPassword123!' }).expect(200);
    // login with new password
    await request(app).post('/api/v1/auth/login').send({ username: 'dave', password: 'NewPassword123!' }).expect(200);
  });

  it('should verify email using token', async () => {
    const signup = await request(app).post('/api/v1/auth/signup').send({ username: 'eve', password: 'Password123!', email: 'eve@example.com' });
    await request(app).post('/api/v1/auth/verify-email').send({ token: signup.body.emailVerificationToken }).expect(200);
